...
function initData() {
	...
	if (table.length > 0) {
		...
		$select_obj.change(function() {
			...
			clearCanvas();
			paintIfs(ls_table['table'][$('.setting-panel .table')[0].selectedIndex]['param'], window.count_default);
		});
...
$(document).ready(function() {
	...
	$('#paint').click(function() {
		...
		paintIfs(ls_table['table'][$('.setting-panel .table')[0].selectedIndex]['param'], draw_count);
	});
	...
	$('#animate').click(function() {
		...
		window.timer = setInterval(function() {
			if (window.animate_count < window.count_default) {
				paintIfs(ls_table['table'][$('.setting-panel .table')[0].selectedIndex]['param'], window.animate_step_default);
